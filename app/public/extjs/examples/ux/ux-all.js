...
Ext.ux.grid.BufferView=Ext.extend(Ext.grid.GridView,{
    ...
    getVisibleRowCount:function(){
        var b=this.getCalculatedRowHeight(),a=this.scroller.dom.clientHeight;
        return(a<1)?0:Math.ceil(a/b)
        },
    getVisibleRows:function(){
        var a=this.getVisibleRowCount(),b=this.scroller.dom.scrollTop,c=(b===0?0:Math.floor(b/this.getCalculatedRowHeight())-1);
        return{
            first:Math.max(c,0),
...
doRender:function(g,k,u,a,s,A,l){
    var b=this.templates,f=b.cell,h=b.row,x=b.rowBody,n=s-1,t=this.getStyleRowHeight(),z=this.getVisibleRows(),d="width:"+this.getTotalWidth()+";height:"+t+"px;",D=[],w,E,v={},m={
        tstyle:d
    },q;
    for(var y=0,C=k.length;y<C;y++){
        q=k[y];
        w=[];
        var o=(y+a),e=o>=z.first&&o<=z.last;
        if(e){
            for(var B=0;B<s;B++){
                E=g[B];
                v.id=E.id;
...
doUpdate:function(){
    if(this.getVisibleRowCount()>0){
        var f=this.grid,b=f.colModel,h=f.store,e=this.getColumnData(),a=this.getVisibleRows(),j;
...
renderer:function(b,c,a){
    ...
    return String.format('<div class="x-grid3-check-col{0} {1}">&#160;</div>',b?"-on":"",this.createId())
    },
createId:function(){
...
Ext.ux.grid.ColumnHeaderGroup=Ext.extend(Ext.util.Observable,{
    ...
    viewConfig:{
        initTemplates:function(){
            ...
            if(!a.gcell){
                a.gcell=new Ext.XTemplate('<td class="x-grid3-hd x-grid3-gcell x-grid3-td-{id} ux-grid-hd-group-row-{row} {cls}" style="{style}">','<div {tooltip} class="x-grid3-hd-inner x-grid3-hd-{id}" unselectable="on" style="{istyle}">',this.grid.enableHdMenu?'<a class="x-grid3-hd-btn" href="#"></a>':"","{value}</div></td>")
                }
                this.templates=a;
...
handleHdMove:function(h,d){
    ...
    if(g&&!this.headersDisabled&&!Ext.fly(g).hasClass("ux-grid-hd-group-cell")){
        var b=this.splitHandleWidth||5,f=this.activeHdRegion,a=h.getPageX(),c=g.style,j="";
...
while(g=g.parentMenu){
    ...
    var n=g.items.findIndexBy(function(r){
        return r.checked
        })>=0;
    c.setChecked(n,true)
    }
...
Ext.ux.tree.ColumnTree=Ext.extend(Ext.tree.TreePanel,{
    ...
    onRender:function(){
        ...
        this.headers=this.header.createChild({
            cls:"x-tree-headers"
        });
...
Ext.DataView.LabelEditor=Ext.extend(Ext.Editor,{
    ...
    constructor:function(a,b){
        Ext.DataView.LabelEditor.superclass.constructor.call(this,b||new Ext.form.TextField({
            allowBlank:false,
            growMin:90,
            growMax:240,
            grow:true,
        ...
        if(this.gmapType==="map"){
            ...
            if(typeof this.setCenter.geoCodeAddr==="string"){
                ...
                if(this.gmapType==="map"){
                    ...
                    if(typeof this.setCenter.marker==="object"&&typeof a==="object"){
...
addAddressToMap:function(a){
    if(!a||a.Status.code!=200){
        Ext.MessageBox.alert("Error","Code "+a.Status.code+" Error Returned")
        }else{
        place=a.Placemark[0];
...
createMenu:function(){
    var a=this.grid.getView(),b=a.hmenu;
    if(this.showMenu&&b){
...
addFilter:function(a){
    var c=this.getFilterClass(a.type),b=a.menu?a:(new c(a));
    this.filters.add(b);
    ...
    for(d=0,a=this.menuItems.length;d<a;d++){
        ...
        if(e!=="-"){
            b={
                itemId:"range-"+e,
                text:this[e+"Text"],
                menu:new Ext.menu.DateMenu(Ext.apply(g,{
                    itemId:e
                })),
                listeners:{
                    scope:this,
...
onMenuSelect:function(c,d,b){
    var a=this.fields,e=this.fields[c.itemId];
    e.setChecked(true);
    if(e==a.on){
...
validateRecord:function(a){
    var b,d,c=a.get(this.dataIndex);
    if(!Ext.isDate(c)){
        return false
        }
...
Ext.ux.grid.filter.NumericFilter=Ext.extend(Ext.ux.grid.filter.Filter,{
    ...
    init:function(a){
        if(this.menu){
            ...
            this.menu=new Ext.ux.menu.RangeMenu(Ext.apply(a,{
...
validateRecord:function(a){
    var c=a.get(this.dataIndex),b=this.getValue();
