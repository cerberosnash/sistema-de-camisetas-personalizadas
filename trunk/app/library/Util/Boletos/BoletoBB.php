...
class BoletoBB {
    ...
    public function __construct($dadosboleto) {
        $_SESSION["dadosboleto"] = $dadosboleto;
        $codigobanco = "001";
        $codigo_banco_com_dv = $this->geraCodigoBanco($codigobanco);
        $nummoeda = "9";
        $fator_vencimento = $this->fator_vencimento($_SESSION["dadosboleto"]["data_vencimento"]);
//valor tem 10 digitos, sem virgula
        $valor = $this->formata_numero($_SESSION["dadosboleto"]["valor_boleto"], 10, 0, "valor");
//agencia � sempre 4 digitos
        $agencia = $this->formata_numero($_SESSION["dadosboleto"]["agencia"], 4, 0);
//conta � sempre 8 digitos
        $conta = $this->formata_numero($_SESSION["dadosboleto"]["conta"], 8, 0);
//carteira 18
        $carteira = $_SESSION["dadosboleto"]["carteira"];
//agencia e conta
...
// Carteira 18 com Conv�nio de 8 d�gitos
        if ($_SESSION["dadosboleto"]["formatacao_convenio"] == "8") {
            $convenio = $this->formata_numero($_SESSION["dadosboleto"]["convenio"], 8, 0, "convenio");
            // Nosso n�mero de at� 9 d�gitos
            $nossonumero = $this->formata_numero($_SESSION["dadosboleto"]["nosso_numero"], 9, 0);
...
// Carteira 18 com Conv�nio de 7 d�gitos
        if ($_SESSION["dadosboleto"]["formatacao_convenio"] == "7") {
            $convenio = $this->formata_numero($_SESSION["dadosboleto"]["convenio"], 7, 0, "convenio");
            // Nosso n�mero de at� 10 d�gitos
            $nossonumero = $this->formata_numero($_SESSION["dadosboleto"]["nosso_numero"], 10, 0);
            $dv = $this->modulo_11("$codigobanco$nummoeda$fator_vencimento$valor$livre_zeros$convenio$nossonumero$carteira");
            $linha = "$codigobanco$nummoeda$dv$fator_vencimento$valor$livre_zeros$convenio$nossonumero$carteira";
...
// Carteira 18 com Conv�nio de 6 d�gitos
        if ($_SESSION["dadosboleto"]["formatacao_convenio"] == "6") {
        ...
        $_SESSION["dadosboleto"]["codigo_barras"] = $linha;
        $_SESSION["dadosboleto"]["linha_digitavel"] = $this->monta_linha_digitavel($linha);
        $_SESSION["dadosboleto"]["agencia_codigo"] = $agencia_codigo;
    ...
    public function output() {
        require_once 'include/dump_bb.php';
    }
...
//Desenho da barra
//Guarda inicial
?><img src=<?php echo BOLETO_PATH; ?>imagens/p.png width=<?php echo $fino ?> height=<?php echo $altura ?> border=0><img
            src=<?php echo BOLETO_PATH; ?>imagens/b.png width=<?php echo $fino ?> height=<?php echo $altura ?> border=0><img
            src=<?php echo BOLETO_PATH; ?>imagens/p.png width=<?php echo $fino ?> height=<?php echo $altura ?> border=0><img
            src=<?php echo BOLETO_PATH; ?>imagens/b.png width=<?php echo $fino ?> height=<?php echo $altura ?> border=0><img
    <?php
        $texto = $valor;
        if ((strlen($texto) % 2) <> 0) {
...
// Draw dos dados
    ...
    ?>
                src=<?php echo BOLETO_PATH; ?>imagens/p.png width=<?php echo $f1 ?> height=<?php echo $altura ?> border=0><img
    <?php
                if (substr($f, $i, 1) == "0") {
                    $f2 = $fino;
                } else {
    ...
    ?>
                src=<?php echo BOLETO_PATH; ?>imagens/b.png width=<?php echo $f2 ?> height=<?php echo $altura ?> border=0><img
    <?php
            }
        }
...
// Draw guarda final
    ?>
        src=<?php echo BOLETO_PATH; ?>imagens/p.png width=<?php echo $largo ?> height=<?php echo $altura ?> border=0><img
        src=<?php echo BOLETO_PATH; ?>imagens/b.png width=<?php echo $fino ?> height=<?php echo $altura ?> border=0><img
        src=<?php echo BOLETO_PATH; ?>imagens/p.png width=<?php echo 1 ?> height=<?php echo $altura ?> border=0>
    <?php
    }
